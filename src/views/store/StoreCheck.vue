    ...
    <div>
        ...
        padding: 5px;">
            ...
            <div @click="goStore" style="float: left">
                &nbsp;&nbsp;<van-icon name="arrow-left" style="float: left; margin-top: 15px" />
            </div>
            <!-- 搜索框 -->
            <div style="float: left; margin-top: 14px">
                <van-search v-model="productName" shape="round" background="#1989fa" placeholder="请输入商品名称"
                    style="width: 220px; height: 10px" @search="onSearch" />

            </div>
            <van-col span="3" style="margin-top: 12px; text-align: left; font-size: 24px;margin-left: -5px">
        ...
        <van-row style="height: 65px; padding: 20px">
            <van-col span="24">
                <!-- 店铺图片 -->
                <van-image width="70" height="70" :src="selectPeopleAndRating.storeImg" radius="8px"
                    style="float: left"></van-image>
                <div style="margin-left: 85px">
                    ...
                    <div style="float: left;margin-top: -20px">
                        <van-rate v-model="selectPeopleAndRating.rating" :size="20" color="#ffd21e" void-icon="star"
                            void-color="#eee" allow-half readonly></van-rate>
                    </div>
                </div>
            </van-col>
        </van-row>
        <!--          activeTab 不根据索引匹配，根据名称匹配-->
        <van-tabs v-model="activeTab" color="#1989FA" title-active-color="#1989FA" @change="handleTabChange" name="a">
            <!-- 标签页 -->
            <van-tab title="综合" name="a">
                <van-grid :border="false" :column-num="2">
                    <van-grid-item v-for="product in products" :key="product.productId" @click="goCommodity(product)">
            ...
            <van-tab title="销量" name="b" @change="handleTabChange">
                <van-grid :border="false" :column-num="2">
                    <van-grid-item v-for="product in products" :key="product.productId" @click="goCommodity(product)">
            ...
            <van-tab title="价格" name="c" @change="handleTabChange">
                <template #title>
                    <div class="tab-title">
                        <span>价格</span>
                        <van-icon class="tab-icon" :name="sortDirection ? 'arrow-up' : 'arrow-down'"
                            @click="priceSort" />
                    </div>
                </template>
    ...
    methods: {
        ...
        optionProductByStoreId() {
            ...
            if (dataParam) {
                ...
                //查询商品列表
                axios.get("/store/check/selectAllProduckByStoreIdOrProductName?productName=" + this.productName + "&storeId=" + storeIdData.storeId)
                    .then(response => {
        ...
        selectPeopleAndRatingByStoreId() {
            ...
            axios.get("/store/product/selectTbStoreEvaluate/" + this.product.storeId)
                .then(resp => {
