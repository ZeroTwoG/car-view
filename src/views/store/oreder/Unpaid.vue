                ...
                <p style="font-size: 10px">
                  ...
                  >
                            ...
                            <van-button
                                    ...
                                    size="small"
                                    @click="del(item.id)"
                                    v-if="item.statusCode!==2&&item.statusCode!==3"
                            >删除订单
                            </van-button
                            >
                            <van-button
                                    round
                                    style="background: #019fe8; color: #ffffff"
                                    size="small"
                                    @click="fk(item)"
                            >{{ formatText(item.statusCode) }}
                            </van-button
                            >
                        </template>
                    </van-card>
        ...
        methods: {
            ...
            del(id) {
                Dialog.confirm({
                    ...
                    this.$http.delete(`/order/api/deleteByOrderId/${id}`).then((response => {
                        if (response.data.code == 200) {
                            Toast.success("删除成功")
                            this.loading = true;
                            this.finished = false;
