      ...
      <span style="
        ...
        >
            <div class="content">免运费,部分地区不包邮</div>
                        <div class="content">{{product.freight}}</div>
        </van-action-sheet>
        <!-- 弹框 -->
        <van-action-sheet
                v-model="show1"
        ...
        <van-row style="height: 45px; padding: 10px 16px">
            ...
            <van-col
                    @click="goEvaluate"
                    span="8"
                    style="text-align: right; font-size: 16px; width: 50%; float: right"
            >好评率{{goodRatingsPercentage}}%
                <van-icon name="arrow"/>
            </van-col>
        </van-row>
        <!-- 评价内容 -->
...
<!--                     头像-->
        ...
        <van-tabbar style="height: 50px">
            ...
            <van-tabbar-item style="background-color: #fc5531; color: white"><span
                    style="font-size: 18px"
                    @click="goOrder(product)"
            >立即兑换</span></van-tabbar-item>
        </van-tabbar>
    </div>
</template>
