<template>
  <div>
    ...
    <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
      ...
      <van-sticky>
        <!-- 标题 -->
        <van-nav-bar title="余额明细" left-arrow @click-left="onClickLeft" />
      </van-sticky>
      <!-- 空白 -->
      <van-tabs v-model="active">
        <van-tab title="全部">
          ...
          <van-row style="font-size: 18px; font-weight: 550" v-for="item in integralLog" justify="space-around"
            ...
            <!-- 充值时间 -->
            <van-col span="13" style="margin-left: 15px" v-if="item.accountMoney >= 1">{{ item.rechargeTime }}
            </van-col>
            <van-col span="3" v-if="item.accountMoney >= 1"></van-col>
            <van-col span="6" style="color: green" v-if="item.accountMoney >= 1">+{{ item.accountMoney }}</van-col>
            <!-- 支出时间 -->
            <van-col span="13" style="margin-left: 15px" v-if="item.accountMoney <= 0">{{ item.expenditureTime }}
            </van-col>
            <van-col span="3" v-if="item.accountMoney <= 0"></van-col>
            <van-col span="6" style="color: red" v-if="item.accountMoney <= 0">- {{ item.expenditure }}
            </van-col>
            <van-cell>
              <hr />
            </van-cell>
        ...
        <van-tab title="收入记录">
          ...
          <van-row style="text-align: left; font-size: 18px; font-weight: 700" v-for="item in incomeInfo"
            ...
            <van-col span="3" v-if="item.accountMoney >= 1"></van-col>
            <van-col span="6" style="color: green" v-if="item.accountMoney >= 1">+ {{ item.accountMoney }}
            </van-col>
            <van-cell>
              <hr />
            </van-cell>
        ...
        <van-tab title="支出记录">
          ...
          <van-row style="text-align: left; font-size: 18px; font-weight: 700" v-for="item in expendInfo"
            :key="item.balanceLogId">
            <van-cell>
              <span style="font-size: 18px; font-weight: 700" v-if="item.accountMoney <= 0">{{ item.storeName }}</span>
            </van-cell>
            <!-- 充值时间 -->
            <van-col span="13" style="margin-left: 15px" v-if="item.accountMoney <= 0">{{ item.expenditureTime }}
            </van-col>
...
export default {
  ...
  methods: {
    init() {
      ...
      axios.get("/my/userBalance/selectBalanceDetail?userId=" + this.userId).then((res => {
        if (res.data.code === 200) {
          ...
          if (this.expendInfo.length >= 1) {
            for (let i = 0; i < this.expendInfo.length; i++) {
              //格式化时间
              let a = this.expendInfo[i].expenditureTime;
              this.expendInfo[i].expenditureTime = moment(a).format("YYYY-MM-DD HH:mm:ss");
            }
            this.flag2 = false;
          } else {
            this.flag2 = true;
